...
CACHE_DIR = Path("data/slowfast_midlevel_fusion_soft_hierarchical")
KINE_EMBED_DIR = CACHE_DIR / "kine_embeddings"
CACHE_DIR.mkdir(parents=True, exist_ok=True)
KINE_EMBED_DIR.mkdir(parents=True, exist_ok=True)

FUSION_META_PATH = CACHE_DIR / "midlevel_fusion_soft_hierarchical_splits.csv"
STAGE1_MODEL_PATH = CACHE_DIR / "stage1_sce_binary_best.pth"
STAGE2_MODEL_PATH = CACHE_DIR / "stage2_sce_type_best.pth"
CALIBRATION_PATH = CACHE_DIR / "soft_hierarchy_calibration.json"
PRED_PATH = CACHE_DIR / "soft_hierarchical_test_predictions.csv"
...
def export_kine_embeddings(df: pd.DataFrame, X_ts: np.ndarray, X_ctx: np.ndarray) -> pd.DataFrame:
    ...
    print(f"Saved soft hierarchical fusion metadata to: {FUSION_META_PATH}")
    return out_df
...
def train_single_model(
    ...
    model = FusionClassifier(num_classes=num_classes).to(DEVICE)
...
def collect_probs_for_split(
    ...
    loader = DataLoader(MidLevelFusionDataset(split_df, target_col="target_idx"), batch_size=BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=False)
    ...
    with torch.no_grad():
        for batch_v, batch_k, batch_targets, batch_ids in loader:
            ...
            stage1_probs = torch.softmax(stage1_model(batch_v, batch_k), dim=1).cpu().numpy()
            stage2_probs = torch.softmax(stage2_model(batch_v, batch_k), dim=1).cpu().numpy()
                ...
                rows.append(
                    {
                        ...
                        "p_not_sce": float(stage1_probs[i, 0]),
                        "p_sce": float(stage1_probs[i, 1]),
                        "p_conflict_given_sce": float(stage2_probs[i, 0]),
                        "p_bump_given_sce": float(stage2_probs[i, 1]),
                        "p_hard_brake_given_sce": float(stage2_probs[i, 2]),
                    }
                )
...
def combine_probs(
    ...
    p_not = np.clip(out["p_not_sce"].to_numpy(dtype=float), 1e-8, 1.0)
    p_sce = np.clip(out["p_sce"].to_numpy(dtype=float), 1e-8, 1.0)
    p_conf = np.clip(out["p_conflict_given_sce"].to_numpy(dtype=float), 1e-8, 1.0)
    p_bump = np.clip(out["p_bump_given_sce"].to_numpy(dtype=float), 1e-8, 1.0)
    p_hb = np.clip(out["p_hard_brake_given_sce"].to_numpy(dtype=float), 1e-8, 1.0)

    # Soft hierarchy: keep Stage 1 probabilistic instead of hard gating.
    not_score = w_not * np.power(p_not, not_sce_bias)
    sce_gate = w_sce * np.power(p_sce, sce_power)

    conflict_score = sce_gate * p_conf
    bump_score = sce_gate * p_bump
    hard_brake_score = sce_gate * p_hb

    score_mat = np.stack([conflict_score, bump_score, hard_brake_score, not_score], axis=1)
    pred_idx = np.argmax(score_mat, axis=1)

    denom = np.sum(score_mat, axis=1, keepdims=True)
    denom = np.maximum(denom, 1e-8)
    prob_mat = score_mat / denom

    out["score_conflict"] = score_mat[:, 0]
    out["score_bump"] = score_mat[:, 1]
    out["score_hard_brake"] = score_mat[:, 2]
    out["score_not_sce"] = score_mat[:, 3]

    out["prob_conflict"] = prob_mat[:, 0]
    out["prob_bump"] = prob_mat[:, 1]
    out["prob_hard_brake"] = prob_mat[:, 2]
    out["prob_not_sce"] = prob_mat[:, 3]

    out["pred_idx"] = pred_idx.astype(int)
    out["pred_label"] = [CLASS_MAP_STR[i] for i in pred_idx]
    out["true_label"] = [CLASS_MAP_STR[i] for i in out["true_idx"].to_numpy(dtype=int)]
    return out



def tune_soft_hierarchy(stage1_model: nn.Module, stage2_model: nn.Module, df: pd.DataFrame) -> Dict[str, float]:
    val_probs = collect_probs_for_split(stage1_model, stage2_model, df, split_name="val")
    y_true = val_probs["true_idx"].to_numpy(dtype=int)
    ...
    for w_not in [0.7, 0.85, 1.0, 1.15, 1.3]:
        for w_sce in [0.7, 0.85, 1.0, 1.15, 1.3]:
            for sce_power in [0.5, 0.75, 1.0, 1.25, 1.5]:
                for not_sce_bias in [0.5, 0.75, 1.0, 1.25, 1.5]:
                    combined = combine_probs(
                        val_probs,
                        w_not=w_not,
                        w_sce=w_sce,
                        sce_power=sce_power,
                        not_sce_bias=not_sce_bias,
                    )
                    y_pred = combined["pred_idx"].to_numpy(dtype=int)
                    acc = accuracy_score(y_true, y_pred)
                    if acc > best["val_acc"]:
                        best = {
                            "w_not": float(w_not),
                            "w_sce": float(w_sce),
                            "sce_power": float(sce_power),
                            "not_sce_bias": float(not_sce_bias),
                            "val_acc": float(acc),
                        }
    ...
    print("\n" + "-" * 80)
    print("SOFT HIERARCHY CALIBRATION")
...
def evaluate_soft_hierarchy(stage1_model: nn.Module, stage2_model: nn.Module, df: pd.DataFrame, calibration: Dict[str, float]) -> None:
    test_probs = collect_probs_for_split(stage1_model, stage2_model, df, split_name="test")
    pred_df = combine_probs(
        test_probs,
        w_not=calibration["w_not"],
        w_sce=calibration["w_sce"],
        sce_power=calibration["sce_power"],
        not_sce_bias=calibration["not_sce_bias"],
    )
    ...
    print("\n" + "=" * 80)
    print("SOFT HIERARCHICAL TEST RESULTS")
    ...
    print(f"Saved soft hierarchical test predictions to: {PRED_PATH}")
    print("=" * 80)
...
def main() -> None:
    ...
    calibration = tune_soft_hierarchy(stage1_model, stage2_model, df)
    evaluate_soft_hierarchy(stage1_model, stage2_model, df, calibration)
