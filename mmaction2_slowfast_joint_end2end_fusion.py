...
# CONFIG
# =========================================================
# Use your current notebook artifacts directly.
KINE_X_PATH = Path("data/X_ts.npy")
KINE_META_PATH = Path("data/meta.csv")
KINE_CTX_PATH = Path("data/X_ctx.csv")
BDD_SCE_PATH = Path("data/bdd_sce.csv")  # optional; used only if you want to inspect richer labels
DOWNLOADED_META_PATH = Path("data/downloaded_videos_meta.csv")  # optional

# Raw .mov videos used for training.
VIDEO_ROOT = Path("data/annotated_videos_only")

# MMAction2 SlowFast config/checkpoint.
...
# Keep backbone fully trainable by default.
FREEZE_VIDEO_BACKBONE = False
UNFREEZE_STAGE = None  # e.g. 'slow_path.layer4' if you later want partial tuning
...
class KinematicsBranch(nn.Module):
    ...
    def forward(self, x_ts: torch.Tensor, x_ctx: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat((x_ts_features, x_ctx_features), dim=1)  # [B, 164]
...
class MMAction2SlowFastFeatureExtractor(nn.Module):
    ...
    def __init__(self, config_path: Path, checkpoint_path: Path, freeze_backbone: bool = False) -> None:
        ...
        # Make checkpoint loading explicit.
    ...
    def _extract_backbone_features(self, video_inputs: torch.Tensor):
        if video_inputs.ndim != 6:
            raise ValueError(
                f"Expected [B, N, C, T, H, W], got {tuple(video_inputs.shape)}"
            )
...
def build_video_path(bdd_id: str) -> Optional[str]:
    """Resolve your .mov files inside annotated_videos_only."""
    candidates = [
...
def load_and_align_data() -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    ctx_features = ctx.drop(
        columns=[c for c in ["BDD_ID", "EVENT_ID", "EVENT_TYPE", "y"] if c in ctx.columns]
    )
...
class SimpleSlowFastVideoLoader:
    """Decode a video into [N, C, T, H, W] without any cached embeddings."""
    ...
    def _sample_frame_indices(self, total_frames: int) -> np.ndarray:
        ...
        if total_frames <= span:
            idx = np.linspace(0, max(total_frames - 1, 0), num=CLIP_LEN, dtype=int)
        else:
            start = max((total_frames - span) // 2, 0)
            idx = start + np.arange(CLIP_LEN) * FRAME_INTERVAL
    ...
    def __call__(self, video_path: str) -> torch.Tensor:
        ...
        frame_inds = self._sample_frame_indices(total_frames)
        ...
        return packed["inputs"].float()  # [N, C, T, H, W]
...
class JointFusionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        video_inputs = self.video_loader(video_path)
        x_ts = self.X_ts[idx]
